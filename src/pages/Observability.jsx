...
import {
  ...
  Zap, Database, Cloud, Cpu, Loader2
} from 'lucide-react';
...
function mapPipeline(p) {
  const status = p.status === 'active' ? 'running'
    : p.status === 'error'  ? 'error'
    : p.status === 'paused' ? 'warning'
    : p.status === 'completed' ? 'completed'
    : 'warning';

  const recs = p.records_processed
    ? p.records_processed >= 1000
      ? `${(p.records_processed / 1000).toFixed(1)}K/min`
      : `${p.records_processed}/min`
    : '—';
  ...
  return {
    ...
    status,
    records: recs,
    health: status === 'running' ? 95 : status === 'error' ? 20 : status === 'completed' ? 100 : 72,
  };
}
...
export default function Observability() {
  ...
  useEffect(() => {
    Pipeline.list('-created_date', 50)
      .then(data => setPipelines(data?.length > 0 ? data.map(mapPipeline) : FALLBACK_PIPELINES))
  ...
  const systemStatus  = errorCount > 0 ? 'Degraded' : 'Operational';

  return (
    <div className="min-h-screen bg-[#050505]">
      <Header title="Observability" subtitle="Real-time platform monitoring" />

      <div className="p-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            { label: 'System Status', value: loading ? '…' : systemStatus, icon: CheckCircle, color: errorCount > 0 ? 'text-amber-400' : 'text-emerald-400' },
            ...
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.05 }} className="glass rounded-xl p-5">
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={performanceData}>
                  ...
                  <Area type="monotone" dataKey="latency" stroke="#ffffff" strokeWidth={2}
                    fill="url(#latencyGrad)" name="Latency (ms)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-medium">
                Active Pipelines
                {!loading && <span className="ml-2 text-xs text-white/30">({runningCount} running)</span>}
              </h3>
              {loading && <Loader2 className="w-4 h-4 text-white/30 animate-spin" />}
            </div>
            <div className="space-y-3">
              {pipelines.slice(0, 6).map((pipeline, i) => (
                <motion.div key={pipeline.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-white/80 truncate max-w-[120px]">{pipeline.name}</span>
                    </div>
                    <span className="text-xs text-white/40 flex-shrink-0">{pipeline.records}</span>
                  </div>
                  <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${pipeline.health >= 90 ? 'bg-emerald-500' : 'bg-amber-500'}`}
              ...
              {pipelines.length === 0 && !loading && (
                <p className="text-xs text-white/30 text-center py-4">
                  Aucun pipeline — créez-en un dans Connectors.
                </p>
              )}
            </div>
          </motion.div>
          ...
          <div className="grid md:grid-cols-4 gap-4">
            {[
              ...
              <div key={service.name} className="p-4 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <div className="space-y-3">
                  {[{ label: 'CPU', val: service.cpu }, { label: 'Memory', val: service.memory }].map(({ label, val }) => (
                    <div key={label}>
                      ...
                      <div className="h-1 bg-white/5 rounded-full">
                        <div className={`h-full rounded-full ${val > 80 ? 'bg-amber-500' : 'bg-white/40'}`}
                          style={{ width: `${val}%` }} />
                      </div>
                    </div>
                  ))}
